...
MedicalSheetServices us ;
        ...
        call.enqueue(new Callback<JsonObject>() {
            ...
            public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                if(response.isSuccessful()){
                    Visite v = new Visite(o.getMedicaments(), com.example.healthsheet.Ordonnance.curanalyse.getFilename());
                    creervisiste(v);
                }
            }
            @Override
    ...
    public void creervisiste(Visite v)
    {
        Gson g = new Gson();
        String j = g.toJson(v);
        JsonParser jp = new JsonParser();
        JsonObject jo = (JsonObject) jp.parse(j);
        Call<JsonObject> call= us.createVisite(jo);
